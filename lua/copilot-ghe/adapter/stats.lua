...
---@return table|nil
local function get_statistics(adapter)
  log:debug("Copilot GHE Adapter: Fetching Copilot usage statistics")

  local oauth_token = token.fetch({ force = true, adapter = adapter }).oauth_token

  local _, _, stats_endpoint = token.resolve_endpoints(adapter)
  local ok, response = pcall(function()
    return Curl.get(stats_endpoint, {
      sync = true,
      headers = {
  ...
  if not ok then
    log:error("Copilot GHE Adapter: Could not get stats: %s", response)
    return nil
  end

  local ok, json = pcall(vim.json.decode, response.body)
  if not ok then
    log:error("Copilot GHE Adapter: Error parsing stats response: %s", response.body)
    return nil
  end
...
---@param adapter? CodeCompanion.HTTPAdapter
---@return nil
function M.show(adapter)
  local stats = get_statistics(adapter)
  if not stats then
